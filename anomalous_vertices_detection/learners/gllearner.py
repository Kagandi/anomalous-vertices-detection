...
import numpy as np
from anomalous_vertices_detection.utils.turi_utils import split_stratified_kfold, get_classification_metrics, \
    cross_validate
...
class GlLearner(AbstractLearner):
    ...
    def cross_validate(self, dataset, n_folds=10):
        ...
        return cross_validate(folds,
                             self._base_classifier,
                             params)

    # def cross_validate(self, dataset, n_folds=10):
    #     roc_auc, recall, precision, accuracy, fpr, tpr, tnr = [], [], [], [], [], [], []
    #     for train, test in split_stratified_kfold(dataset.features, n_folds=n_folds):
    #         f_train, f_test = DataSet(train, 'label'), DataSet(test, 'label')
    ...
    #         metrics = cross_val.get_classification_metrics(f_test.features['label'], prediction)
    #         recall.append(metrics["recall"])
    #         precision.append(metrics["precision"])
    #         accuracy.append(metrics["accuracy"])
    #         roc_auc.append(metrics["auc"])
    #     return {"auc": np.mean(roc_auc), "accuracy": np.mean(accuracy), "precision": np.mean(precision), "recall": np.mean(recall)}
